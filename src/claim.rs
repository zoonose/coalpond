...
pub async fn claim(args: ClaimArgs, key: Keypair, url: String, unsecure: bool) {
    let mut claim_amount = (args.amount * 10f64.powf(coal_api::consts::TOKEN_DECIMALS as f64)) as u64;
    ...
    loop {
        ...
        let balance_grains = (balance.parse::<f64>().unwrap() * 10f64.powf(coal_api::consts::TOKEN_DECIMALS as f64)) as u64;
        println!("Claimable Rewards: {} COAL", balance);
        ...
        if claim_amount > balance_grains {
            println!("You do not have enough rewards to claim {} COAL.", amount_to_ui_amount(claim_amount, coal_api::consts::TOKEN_DECIMALS));
            println!("Please enter an amount less than or equal to {} COAL.", amount_to_ui_amount(balance_grains, coal_api::consts::TOKEN_DECIMALS));
            claim_amount = loop {
                let mut input = String::new();
                io::stdout().flush().unwrap();
                let _ = std::io::stdin().read_line(&mut input);
                if let Ok(new_amount) = input.trim().parse::<f64>() {
                    let new_claim_amount = (new_amount * 10f64.powf(coal_api::consts::TOKEN_DECIMALS as f64)) as u64;
        ...
        if !ask_confirm(
            format!(
                ...
                format!(
                    "{} COAL",
                    amount_to_ui_amount(claim_amount, coal_api::consts::TOKEN_DECIMALS)
                )
            )
            .as_str(),
        ) {
